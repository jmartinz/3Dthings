...
//Prusa i3 Carriage mount separation (24 or 30)
seps = 30;
...
//altura extra para pasar tornillos motor directo con reductora (mm)
extraelevation = 8;

//grueso extra para budas en i3
extrag = 9;
...
wade(hotend_mount=buddaschnozzle, legacy_mount=false);
...
////////// RENDER IDLER /////////////////////////////////////////////////////////////////////
translate([54+3,0,16.20])		// POSITIONING
	rotate([0,-90,0])
		wadeidler();
...
wade_block_height=55+elevation+extraelevation;
wade_block_width=24;
wade_block_depth=28;
...
idler_long_top=idler_mounting_hole_up+idler_mounting_hole_diameter/2+idler_mounting_hole_elongation+2.5+extraelevation;
...
module wade (hotend_mount=0,legacy_mount=false){
	difference (){
		union(){
			//mod Chema
			translate([0,0,0]) 
			cube([wade_block_width,wade_block_height,wade_block_depth+extrag]);
			// The wade block.
			//cube([wade_block_width,wade_block_height,wade_block_depth]);

// Carriage mount cylinders
 translate([11,-15,0]){
  translate([-seps/2,24,0]) cylinder(r=7, h=wade_block_depth+extrag);
	translate([-seps/2,25,0]) cube([8,6,wade_block_depth+extrag]);
  translate([seps/2,24,0]) cylinder(r=7, h=wade_block_depth+extrag);
	translate([seps/2-8,25,0]) cube([8,6,wade_block_depth+extrag]);
	translate([-seps/2-7,15,0]) cube([seps+14,9,wade_block_depth+extrag]);
}
		...
		rotate([-90,0,0]){
			...
			//if (in_mask(hotend_mount,buddaschnozzle))
				//no hole for the buddasch
		}
	}
}
...
*/		
			...
			//translate([-13,0,9.5+8+layer_thickness])
			...
			translate([0,0,-20])
			cylinder(r=16/2,h=wade_block_depth+40);	

  // Lower motor mount hole
  translate([8.5,18-2,-1]) cylinder(r=2, h=35);
  translate([8.5,18-2,21-extrag]) cylinder(r=3.1, h=35);
...
// Carriage mount cylinders
 translate([0,-53,0]){
  //translate([-12,24,0]) cylinder(r=5, h=wade_block_depth);
  //translate([seps/2,24,0]) cylinder(r=5, h=wade_block_depth);

  // Carriage mount right screw head hole
  translate([-seps/2,24,-3]) cylinder(r=3.5, h=22+extrag);
  // Carriage mount left screw head hole
  translate([seps/2,24,-3]) cylinder(r=3.5, h=22+extrag);
  // Carriage mount right screw hole
  translate([-seps/2,24,20.52-extrag]) cylinder(r=2, h=23+extrag);
  // Carriage mount left screw head hole
  translate([seps/2,24,20.5-extrag]) cylinder(r=2, h=23+extrag);
}
...
//%		translate([0,0,-8])
	...
	for (idle=[-1,1]){
		translate([0,
			idler_mounting_hole_up+motor_mount_translation[1]+extraelevation,
...
//%cube([12,4,5],center=true);
	...
	guide_height=12.3;
	//Chema mod
	guide_length=10+extraelevation;
...
//		rotate([0,0,30])
		...
		translate(idler_axis+[2-idler_height,0,0]){
			//Screw Holes. Schraubenloecher. Chema mod
			translate([-1,idler_mounting_hole_up-1+extraelevation,
...
module motor_dummy(){
	...
	translate([18,18,-5]) %cylinder(r=2, h=44);
	translate([18,-18,-5]) %cylinder(r=2, h=44);
	translate([-18,18,-5]) %cylinder(r=2, h=44);
	translate([-18,-18,-5]) %cylinder(r=2, h=44);
	translate([-22+2.5+10,0,-10]) %cylinder(r=2.5, h=50);
	
}


//rotate([180,0,180]) translate([-1,38,-28]) motor_dummy();
//translate ([-20,-5,10]) %cube([50,5,5]);
