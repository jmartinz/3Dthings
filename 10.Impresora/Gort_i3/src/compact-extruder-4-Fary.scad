...
module extruder_base(){
 ...
 color("green") translate([-19,49,0]) cube([72,5,24]); // -> JMMP
 }
...
module extruder_holes(){
 ...
 translate([1.5+11+3.5+27,65,11]) rotate([90,0,0]) cylinder(r=1.8, h=70);// JMMP
 ...
 translate([58,46,-1]) rotate([0,0,45]) cube([20,20,26]); // JMMP
 translate([46,13,-1]) rotate([0,0,45]) cube([20,20,26]);
 translate([-25,46,-1]) rotate([0,0,45]) cube([20,20,26]);//JMMP
