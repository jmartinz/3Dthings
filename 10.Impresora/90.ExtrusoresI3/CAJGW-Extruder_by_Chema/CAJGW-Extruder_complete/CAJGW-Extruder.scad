// Better Geared Extruder: CAJGW - Chema/Alain/Jonas/Greg/Wade
// by Chema, apr-2013
...
include <x-carriageV2.1.scad>
include <CAJGW-GearsHering.scad>
include <CAJGW-FastRelease.scad>
...
show_support=false;				// Show Support structures (needed fo printing)
...
it3=true; 					//for Prusa it3, slots to screw at vertical x-carriage
extrag=9; 					//how much extra width for Budas style on it3 (9)
seps=30; 					//space for it3 holes (24 or 30mm)
...
//wade(hotend_mount=budas, legacy_mount=false);
...
/*rotate([0,0,0])
translate([20+2.3,-30-1,28+extrag])
x_carriage();*/

////////// RENDER motor dummy /////////////////////////////////////////////////////////////////////

 //translate([28-3+2,47+3.8+elevation,58]) rotate([180,0,motor_mount_rotation]) %motor_dummy();
 //translate([10,16,50])rotate([0,90,0]) %shaft();
 //translate([10.7,48,-30])rotate([0,0,0]) %shaft(70,false);

////////// RENDER gears /////////////////////////////////////////////////////////////////////
/*translate([10.7,48,-30+22]) rotate([0,180,0])
WadeL_double_helix();

translate([10.7+42,48,-30+22]) rotate([0,0,14]) WadesS_double_helix();*/

////////// RENDER quick lever /////////////////////////////////////////////////////////////////////

translate([-26,48+16,10]) 
lever();
translate([-23-6,48+16,0]) rotate([0,0,180]) bar();
...
motor_mount_rotation=12;   //25 [am] allow clearing for top mount hex key
...
/* Moved at the end of everything
	...
	translate(motor_mount_translation){
		translate([-gear_separation,0,0]){
			...
			rotate([90,0,0])
			rotate(360/16)
			cylinder(r=filament_feed_hole_d/2,h=(wade_block_depth+elevation)*3,center=true,$fn=16);
...
module budas_holes (){
	...
	// Recess in base nothing to do with budas
	//translate([0,0,-1])
	//%cylinder(r=extruder_recess_d/2,h=extruder_recess_h+1);
}
...
module cmcy(){
	difference(){
		union(){
		  	translate([-seps/2,24,0]) cylinder(r=6.3, h=wade_block_depth+extrag);
			translate([-seps/2,25,0]) cube([18,5.3,wade_block_depth+extrag]);
		  	translate([seps/2,24,0]) cylinder(r=6.3, h=wade_block_depth+extrag);
			translate([seps/2-8-10,25,0]) cube([18,22,wade_block_depth+extrag]);
			translate([-seps/2-7+0.7,20,0]) cube([seps+14-1.4,5,wade_block_depth+extrag]);
		}
		translate([-30,0,0])
		cube([seps*2,20,wade_block_depth+extrag]);
	}
}
...
module cmcyh(){
	  ...
	  translate([-seps/2,24,-3]) cylinder(r=3.5, h=10+extrag);
	  // Carriage mount left screw head hole
	  translate([seps/2,24,-3]) cylinder(r=3.5, h=10+extrag);
	  // Carriage mount right screw hole
	  translate([-seps/2,24,20.52-extrag]) cylinder(r=2, h=23+20+extrag);
	  // Carriage mount left screw head hole
	  translate([seps/2,24,20.5-extrag]) cylinder(r=2, h=23+20+extrag);
...
module motor_dummy(){
	difference(){
			union(){
			cube([nema17_width,nema17_width,48]);
			translate([(nema17_width-nema17_hole_spacing)/2,(nema17_width-nema17_hole_spacing)/2,15]) cylinder(r=1.5, h=44);
			translate([(nema17_width-nema17_hole_spacing)/2+nema17_hole_spacing,(nema17_width-nema17_hole_spacing)/2,15]) cylinder(r=1.5, h=44);
			translate([(nema17_width-nema17_hole_spacing)/2,(nema17_width-nema17_hole_spacing)/2+nema17_hole_spacing,15]) cylinder(r=1.5, h=44);
			translate([(nema17_width-nema17_hole_spacing)/2+nema17_hole_spacing,(nema17_width-nema17_hole_spacing)/2+nema17_hole_spacing,15]) cylinder(r=1.5, h=44);
			translate([nema17_width/2,nema17_width/2,20]) cylinder(r=2.5, h=44);
			}
			rotate([0,0,45]) translate([-6,-5,-1]) cube([10,10,50]);
			translate([nema17_width,-6,-1]) rotate([0,0,45]) cube([10,10,50]);
			translate([nema17_width,-6+nema17_width,-1]) rotate([0,0,45]) cube([10,10,50]);
			translate([0,-6+nema17_width,-1]) rotate([0,0,45]) cube([10,10,50]);
	}
	
}

module shaft(long=100,bearing=true){

cylinder(r=4,h=long,$fn=30);
if (bearing) cylinder(r=7.5,h=20,$fn=30);
