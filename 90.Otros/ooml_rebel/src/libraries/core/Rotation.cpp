...
string Rotation::gen_scad(int indent) const
{
	 stringstream out;

    stringstream id;
	 id << "rotate([" << fixed << ax << "," << ay << "," << az << "]) {";
...
void Rotation::print_ast(int indent) const
{
    stringstream id;

    id << "ROTATION(" << fixed << ax << "," << ay << "," << az << ") ";
    UniObject::print_ast(id.str(),indent);
}

Rotation& Rotation::clone() const
{
    debug("Rotation: clone!");
